  ...
  return (
    <form action={handleSubmit} className="space-y-8 rounded-2xl bg-white p-8 shadow-xl shadow-gray-200/50 border border-gray-100">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full rounded-xl bg-black px-6 py-4 text-base font-bold text-white transition hover:bg-gray-800 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading ? "Publicando e Enviando Arquivos..." : "Publicar Projeto Agora"}
      </button>
    </form>
  )
}